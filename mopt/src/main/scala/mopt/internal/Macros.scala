...
class Macros(val c: blackbox.Context) {
  ...
  def deriveJsonCodecImpl[T: c.WeakTypeTag](default: Tree): Tree = {
    val T = assumeClass[T]
    q"""
        _root_.mopt.JsonCodec.EncoderDecoderToCodec[$T](
          _root_.mopt.generic.deriveEncoder[$T],
          _root_.mopt.generic.deriveDecoder[$T]($default)
        )
     """
  }
  ...
  def deriveJsonEncoderImpl[T: c.WeakTypeTag]: Tree = {
    ...
    val writes = params.map { param =>
      ...
      val encoder =
        q"_root_.scala.Predef.implicitly[_root_.mopt.JsonEncoder[${param.info}]]"
      q"($name, $encoder.write($select))"
    }
    val result = q"""
       new ${weakTypeOf[JsonEncoder[T]]} {
         override def write(value: ${weakTypeOf[T]}): _root_.mopt.Conf = {
           new _root_.mopt.Conf.Obj(
  ...
  def deriveJsonDecoderImpl[T: c.WeakTypeTag](default: Tree): Tree = {
    ...
    if (paramss.head.isEmpty)
      return q"_root_.mopt.JsonDecoder.constant($default)"
    ...
    val result = q"""
       new ${weakTypeOf[JsonDecoder[T]]} {
         def read(
             context: _root_.mopt.DecoderContext
  ...
  def deriveSurfaceImpl[T: c.WeakTypeTag]: Tree = {
    ...
    val argss = ctor.paramLists.map { params =>
      val fields = params.map { param =>
        ...
        val repeated =
          if (isIterable) {
            q"new _root_.mopt.annotation.Repeated" :: Nil
          } else {
            Nil
          }
        val dynamic =
          if (isMap || isConf) {
            q"new _root_.mopt.annotation.Dynamic" :: Nil
          } else {
            Nil
          }
        val flag =
          if (paramTpe <:< typeOf[Boolean]) {
            q"new _root_.mopt.annotation.Flag" :: Nil
          } else {
            Nil
          }
        ...
        val tabCompletePath =
          if (paramTpe <:< typeOf[Path] || paramTpe <:< typeOf[File]) {
            q"new _root_.mopt.annotation.TabCompleteAsPath" :: Nil
          } else {
            Nil
          }
