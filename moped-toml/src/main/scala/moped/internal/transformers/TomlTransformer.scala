...
class TomlTransformer(input: Input) extends AstTransformer[toml.Value] {
  override def transform[T](j: Value, f: Visitor[_, T]): T =
    j match {
      ...
      case Value.Bool(false) =>
        f.visitFalse(-1)
      case Value.Real(value) =>
        f.visitFloat64(value, -1)
      case Value.Num(value) =>
        f.visitInt64(value, -1)
      case Value.Tbl(value) =>
        transformObject(f, value)
      case Value.Arr(value) =>
        transformArray(f, value)
    }
  ...
  override def visitNull(index: Int): Value =
    Value
      .Str("null") // Null isn't supported in TOML so we convert into a String.
